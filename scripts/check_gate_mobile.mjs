...
const catSkillsResult = await call("Runtime.evaluate", { expression:"(() => {bullets=[];fireWeapon({type:'hairball',level:2},{x:player.x+100,y:player.y});const hairball={type:bullets[0].weaponType,pierce:bullets[0].pierce,r:bullets[0].r};bullets=[];fireWeapon({type:'meow',level:2},{x:player.x+100,y:player.y});const meowCount=bullets.length;catLives=1;hp=1;invulnerable=0;damagePlayer(999);kills=9;feverTime=0;handleEnemyDefeat({type:'walker',score:18,x:player.x+200,y:player.y});return {hairball,meowCount,revived:hp===40&&catLives===0,fever:feverTime}})()", returnByValue:true });
...
console.log(JSON.stringify({ fit, controlClearance, ratioMatch, ...value, started, dashState, comboCheck, mapCheck, growth, motion, projectile, bossGrowth, catnip, catSkills, settingsScreen, screenshot:output }));
if (!fit || !ratioMatch || dashState === "OK" || comboCheck.combo !== 2 || mapCheck.a.seed===mapCheck.b.seed || mapCheck.b.stage!==2 || mapCheck.a.obstacles<10 || mapCheck.b.hazards<3 || growth.upgrades.attack!==2 || growth.upgrades.speed!==2 || growth.upgrades.defense!==2 || growth.slots!==3 || growth.radarWeapon!==1 || motion.zoom>=.8 || motion.speed!==205 || motion.directions!==8 || !motion.stopped || motion.phase<=0 || projectile.high.r<=projectile.low.r || projectile.high.visual!==5 || projectile.high.count<=projectile.low.count || !projectile.radar || bossGrowth.hp5<=bossGrowth.hp1 || bossGrowth.phase!==3 || bossGrowth.ring<8 || bossGrowth.homing!==3 || bossGrowth.mines<4 || bossGrowth.summons<2 || catnip.time<7 || !catnip.bulletCleared || catnip.enemyHp>=100 || catSkills.hairball.type!=="hairball" || catSkills.hairball.pierce<5 || catSkills.meowCount<5 || !catSkills.revived || catSkills.fever<4 || !value.headerHidden || !value.settingsHidden || !settingsScreen.open || !settingsScreen.exportVisible || !settingsScreen.radarVisible || settingsScreen.loadout!==3 || started.label !== "やり直す") process.exitCode = 1;
