...
const result = await call("Runtime.evaluate", { expression:`(() => {
  ...
  return {inner:[innerWidth,innerHeight],scroll:[document.documentElement.scrollWidth,document.documentElement.scrollHeight],stage:rect('.stage'),controls:rect('.controls'),start:rect('#mobile-start'),settings:rect('#mobile-settings'),items:[rect('#use-slot-0'),rect('#use-slot-1')],headerHidden:!visible('header'),settingsHidden:!visible('aside')};
})()`, returnByValue:true });
...
console.log(JSON.stringify({ fit, ...value, started, settingsScreen, screenshot:output }));
if (!fit || !value.headerHidden || !value.settingsHidden || !settingsScreen.open || !settingsScreen.exportVisible || started.label !== "やり直す") process.exitCode = 1;
