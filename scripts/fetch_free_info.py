...
def load_schedule():
    default = {
        "cadenceHours": 6,
        "allowedCadenceHours": [1, 3, 6, 12, 24],
        "adaptiveWeather": True,
        "weatherRiskCadenceHours": 1,
        "minCadenceHours": 1,
        "maxCadenceHours": 24,
    }
    ...
    default.update(data)
    allowed = set(default.get("allowedCadenceHours") or [1, 3, 6, 12, 24])
    cadence = int(default.get("cadenceHours") or 6)
    default["cadenceHours"] = cadence if cadence in allowed else 6
    default["weatherRiskCadenceHours"] = max(1, int(default.get("weatherRiskCadenceHours") or 1))
    return default
...
def effective_cadence_hours(schedule):
    if schedule.get("adaptiveWeather") and latest_weather_risk() not in {"normal", "unknown"}:
        return min(int(schedule["cadenceHours"]), int(schedule["weatherRiskCadenceHours"]))
    return int(schedule["cadenceHours"])
...
def should_fetch(now_dt, schedule):
    ...
    cadence = effective_cadence_hours(schedule)
    age_hours = (now_dt - updated).total_seconds() / 3600
    return age_hours >= cadence, f"age {age_hours:.2f}h / cadence {cadence}h"
...
def build_source_catalog(now, sources, schedule):
    ...
    return {
        ...
        "currentWorkflow": {
            "cadence": f"{effective_cadence_hours(schedule)}h",
            "baseCadenceHours": schedule["cadenceHours"],
            "adaptiveWeather": bool(schedule.get("adaptiveWeather")),
            "weatherRiskCadenceHours": schedule.get("weatherRiskCadenceHours"),
...
def main():
    ...
    payload = {
        ...
        "schedule": {
            ...
            "effectiveCadenceHours": effective_cadence_hours(schedule),
            "adaptiveWeather": bool(schedule.get("adaptiveWeather")),
            "weatherRiskCadenceHours": schedule.get("weatherRiskCadenceHours"),
        },
        "contexts": contexts,
